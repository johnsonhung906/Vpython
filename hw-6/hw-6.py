...
while True:
	...
	for i in range(N):
		...
		total_t_E += 0.5*m*(v_a[i][0]**2+v_a[i][1]**2+v_a[i][2]**2)
	if stage == 2 and draw1:
		theory_low_T.color = color.green
		for v in arange(0., 4201. + dv, dv):  # theoretical speed distribution
			theory_low_T.plot(pos=(v, (deltav / dv) * N * 4. * pi * ((m / (2. * pi * k * T)) ** 1.5) * exp(
				(-0.5 * m * v ** 2) / (k * T)) * (v ** 2) * dv))
		observation = ghistogram(graph = vdist, bins=arange(0.,4200.,deltav), color=color.blue)
		draw1 = False
	if stage == 3 and draw2:
	...
	for i in range(N):
		if abs(p_a[i][0]) >= container.length/2 - size and p_a[i][0]*v_a[i][0] > 0:
			prev = v_a[i][0]
			if v_a[i][0] > 0:
				v_a[i][0] = - v_a[i][0] - wall_v
			else:
				v_a[i][0] = - v_a[i][0] + wall_v
			total_m += abs(m * (v_a[i][0] - prev))/(2*container.width*container.height)
		if abs(p_a[i][1]) >= L - size and p_a[i][1]*v_a[i][1] > 0 :
			v_a[i][1] = - v_a[i][1]
			total_m += abs(m * v_a[i][1] * 2)/(2*container.length*container.width)
		if abs(p_a[i][2]) >= L - size and p_a[i][2]*v_a[i][2] > 0:
			v_a[i][2] = - v_a[i][2]
			total_m += abs(m * v_a[i][2] * 2)/(2*container.height*container.length)
	...
	if times%1000 == 0:
		...
		print(f'V:{V}',end=' ')
		print(f'P*V:{P*V}',end=' ')
		print(f'N*k*T:{N*k*T}',end=' ')
		print(f'p*(V**gamma):{P*V**(5/3)}',end='\n')
